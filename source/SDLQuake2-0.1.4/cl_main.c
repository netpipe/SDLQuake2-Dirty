...
{
	//! Cvar_Set ("cl_version", va("SDLQuake2 %s; %s", VERSION, *newValue ? newValue : "unknown renderer"));
}
...
//!	(Cvar_ForceSet ("cl_version", va("SDLQuake2 %s; %s", VERSION, *glVersion ? glVersion : "unknown renderer" )))->changed = version_update;
...
void CL_Frame (int msec)
{
	...
	if(render_frame)
	{
		...
		if(misc_frame)
		{
			...
			{
				char buff[256];
        // advertising on my time ... no		Com_sprintf (buff, sizeof(buff), "say \"SDLQuake2 %s %s %s %s \"\n", VERSION,
		//			__TIMESTAMP__, CPUSTRING, BUILDSTRING   );
				Cbuf_AddText (buff);
				cls.lastSpamTime = cls.realtime;
				cls.spamTime = 0;
